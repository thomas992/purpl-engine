...
static int getkeystring(INI_FILETYPE *fp, const TCHAR *Section, const TCHAR *Key, int idxSection, int idxKey,
			TCHAR *Buffer, int BufferSize, INI_FILEPOS *mark)
{
	...
	if (len > 0 || idxSection >= 0) {
		...
		do {
			...
			ep = _tcsrchr(sp, ']');
			 ...
			 (((int)(ep - sp - 1) != len || Section == NULL || _tcsnicmp(sp + 1, Section, len) != 0) &&
			  ++idx != idxSection));
		if (idxSection >= 0) {
			if (idx == idxSection) {
				assert(ep != NULL);
	...
	do {
		...
		if (ep == NULL)
		 ...
		 ((len == 0 || (int)(skiptrailing(ep, sp) - sp) != len || _tcsnicmp(sp, Key, len) != 0) &&
		  ++idx != idxKey));
	if (idxKey >= 0) {
		if (idx == idxKey) {
			assert(ep != NULL);
 ...
 */
int ini_geti(const mTCHAR *Section, const mTCHAR *Key, int DefValue, const mTCHAR *Filename)
{
	TCHAR LocalBuffer[64];
	int len = ini_gets(Section, Key, __T(""), LocalBuffer, sizearray(LocalBuffer), Filename);
	return (len == 0) ? DefValue :
				  ((len >= 2 && _totupper((int)LocalBuffer[1]) == 'X') ? (int)_tcstol(LocalBuffer, NULL, 16) :
											 (int)_tcstol(LocalBuffer, NULL, 10));
}
 ...
 */
	...
	if (len > 0) {
		do {
			if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
				...
				if (Key != NULL && Value != NULL) {
					if (!flag)
						(void)ini_write(INI_LINETERM, &wfp); /* force a new line behind the last
	...
	for (;;) {
		if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
			...
			if (Key != NULL && Value != NULL) {
				if (!flag)
					(void)ini_write(INI_LINETERM, &wfp); /* force a new line behind the last line of
										the INI file */
				writekey(LocalBuffer, Key, Value, &wfp);
			} /* if */
			return close_rename(&rfp, &wfp, Filename, LocalBuffer); /* clean up and rename */
