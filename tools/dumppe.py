...
existing Command Prompt, or run buildcmd.bat to open one for you.
    ...
    try:
        readpe = subprocess.run(["readpe", "-e", sys.argv[1]], capture_output=True, check=True, encoding="utf-8")
    except:
        print("""
Failed to run readpe. This script requires readpe, which is usually found in
a package named pev, or if not you can compile it from the sources at http://pev.sourceforge.net
              """)

    start_str = "Functions"
    start = readpe.stdout.find(start_str) + len(start_str) + 1
    out = readpe.stdout[start:]
...
if platform.system() == "Windows":
    f.write(bytes(f"#define PREFIX(x) __imp_##x\n\n", encoding="utf-8"))
else:
    f.write(bytes(f"#define PREFIX(x) x\n\n", encoding="utf-8"))

for name in names:
    f.write(bytes(f"EXTERN_C void (*PREFIX({name}))(void) = (void (*)(void))0;\n", encoding="utf-8"))
...
for name in names:
    f.write(bytes(f"\t*(FARPROC *)(&PREFIX({name})) = GetProcAddress(dll, \"{name}\");\n", encoding="utf-8"))
