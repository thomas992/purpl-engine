...
bool vulkan_pick_physical_device(void)
{
	...
	if (!device_count) {
		PURPL_LOG_ERROR(purpl_inst->logger, "Failed to locate a device with Vulkan support");
		return false;
	}
	...
	vkGetPhysicalDeviceFeatures(vulkan->phys_device, &vulkan->phys_device_features);
	vkGetPhysicalDeviceProperties(vulkan->phys_device, &vulkan->phys_device_properties);
	PURPL_LOG_INFO(purpl_inst->logger,
		       "Device %zu (%s, handle 0x%" PRIX64 ") has the best score (its score is %llu)", best_idx + 1,
		       vulkan->phys_device_properties.deviceName, vulkan->phys_device, best_score);
	vulkan_get_device_queue_families(vulkan->phys_device, &vulkan->queue_families);
...
bool vulkan_create_logical_device(void)
{
	...
	if (vulkan->queue_families.graphics_family != vulkan->queue_families.presentation_family) {
		...
		stbds_arrsetlen(queue_create_infos, queue_create_info_count);
		memset(&queue_create_infos[1], 0, sizeof(VkDeviceQueueCreateInfo));

		queue_create_infos[1].sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
		queue_create_infos[1].queueFamilyIndex = (u32)vulkan->queue_families.presentation_family;
	...
	device_create_info.enabledExtensionCount = (u32)stbds_arrlenu(required_extensions);
	device_create_info.ppEnabledExtensionNames = (const char *const *)required_extensions;
	...
	PURPL_LOG_INFO(purpl_inst->logger, "Successfully created logical device with handle 0x%" PRIX64,
		       vulkan->device);

	vkGetDeviceQueue(vulkan->device, (u32)vulkan->queue_families.compute_family, 0, &vulkan->compute_queue);
	PURPL_LOG_INFO(purpl_inst->logger, "Retrieved handle 0x%" PRIX64 " for compute queue",
		       vulkan->presentation_queue);
	vkGetDeviceQueue(vulkan->device, (u32)vulkan->queue_families.graphics_family, 0, &vulkan->graphics_queue);
	PURPL_LOG_INFO(purpl_inst->logger, "Retrieved handle 0x%" PRIX64 " for graphics queue", vulkan->graphics_queue);
	vkGetDeviceQueue(vulkan->device, (u32)vulkan->queue_families.presentation_family, 0,
			 &vulkan->presentation_queue);
	PURPL_LOG_INFO(purpl_inst->logger, "Retrieved handle 0x%" PRIX64 " for presentation queue",
		       vulkan->presentation_queue);
...
u64 vulkan_score_device(VkPhysicalDevice device, size_t idx)
{
	...
	if (!vulkan_get_device_queue_families(device, &queue_families)) {
		PURPL_LOG_INFO(purpl_inst->logger,
			       "Device %zu (%s) does not have the necessary queue families, ignoring", idx + 1,
			       properties.deviceName);
		return 0;
	}

	if (!vulkan_check_device_extensions(device, &extension_count)) {
		PURPL_LOG_INFO(purpl_inst->logger, "Device %zu (%s) does not have the necessary extensions, ignoring",
	...
	if (!vulkan_get_swapchain_info(device, &swapchain_info)) {
		PURPL_LOG_INFO(purpl_inst->logger,
			       "Device %zu (%s) does not have the necessary swapchain info, ignoring", idx + 1,
			       properties.deviceName);
		return 0;
	}

	if (properties.apiVersion < VK_API_VERSION_1_1) {
		PURPL_LOG_INFO(purpl_inst->logger, "Device %zu (%s) does not support Vulkan 1.1, ignoring", idx + 1,
			       properties.deviceName);
		return 0;
	}
	...
	if (queue_families.graphics_family == queue_families.presentation_family)
		score += 100;
	...
	PURPL_LOG_INFO(purpl_inst->logger, "\tPCI ID: %x:%x (%s)", properties.vendorID, properties.deviceID, vulkan_get_device_vendor(properties.vendorID));
	...
	PURPL_LOG_INFO(purpl_inst->logger, "\t\tCompute: %zu", queue_families.compute_family);
	PURPL_LOG_INFO(purpl_inst->logger, "\t\tGraphics: %zu", queue_families.graphics_family);
	PURPL_LOG_INFO(purpl_inst->logger, "\t\tPresentation: %zu", queue_families.presentation_family);
	PURPL_LOG_INFO(purpl_inst->logger, "\tSwapchain information:");
	PURPL_LOG_INFO(purpl_inst->logger, "\t\tSurface format count: %zu", stbds_arrlenu(swapchain_info.formats));
	PURPL_LOG_INFO(purpl_inst->logger, "\t\tPresent mode count: %zu", stbds_arrlenu(swapchain_info.present_modes));
...
const char *vulkan_get_device_vendor(enum vulkan_device_vendor_id id)
{
	switch (id) {
	case VULKAN_DEVICE_VENDOR_AMD:
		return "AMD";
	case VULKAN_DEVICE_VENDOR_INTEL:
		return "Intel";
...
bool vulkan_check_device_extensions(VkPhysicalDevice device, size_t *extension_count)
{
	...
	if (!extension_count) {
		PURPL_LOG_ERROR(purpl_inst->logger, "Failed to locate any device extensions");
		return false;
	}
	...
	vkEnumerateDeviceExtensionProperties(device, NULL, &extension_count_tmp, extensions);

	for (i = 0; i < stbds_arrlenu(extensions); i++) {
		for (j = 0; j < stbds_arrlenu(extension_names); j++) {
			if (strcmp(extensions[i].extensionName, extension_names[j]) == 0) {
				PURPL_LOG_INFO(purpl_inst->logger,
	...
	if (all_found)
		PURPL_LOG_INFO(purpl_inst->logger,
			       "All device extensions (%zu in total) found for device with handle 0x%" PRIX64, found,
			       device);
...
bool vulkan_get_device_queue_families(VkPhysicalDevice device, struct vulkan_queue_families *queue_families)
{
	...
	for (i = 0; i < stbds_arrlenu(queue_family_list); i++) {
		...
		all_found = (queue_families->compute_family_present && queue_families->graphics_family_present &&
