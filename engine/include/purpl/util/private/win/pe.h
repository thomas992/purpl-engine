// Functions and structures for manipulating Portable Executables
//
...
struct dos_mz_header {
	...
	uint16_t min_alloc; // Minimum number of paragraphs for the executable
			    // to function
	...
	uint16_t checksum; // Must equal zero when added to all other words in
			   // the executable
...
struct pe_file_header {
	...
	uint32_t timestamp; // Timestamp of when the image was created. Often 0
			    // or negative.
...
#define PE_DATA_DIR_MAX_SIZE \
	(PE_DATA_DIR_MAX_ENTRIES * sizeof(struct pe_data_directory))

// PE data directory size
#define PE_DATA_DIR_SIZE(entry_count) \
	((entry_count) * sizeof(struct pe_data_directory))
...
// PE optional header. Contains many important values, and several
...
struct pe_optional_header {
...
#if defined PURPL_X86_64 || defined PURPL_ARM
	...
	uint16_t major_system_version; // Major required operating system
				       // version
	uint16_t minor_system_version; // Minor required operating system
				       // version
	...
	uint32_t image_size; // How much space the executable needs to function
			     // when loaded into memory
	uint32_t headers_size; // Size of some of the executable's headers.
			       // Check the docs for IMAGE_OPTIONAL_HEADER for
			       // more.
	uint32_t checksum; // Checksum of the executable. Should probably be
...
// Full PE header. In separate structs because the optional header can
// technically be empty.
struct pe_header {
	uint32_t magic; // Magic signature, must be 0x4550 in little endian
	struct pe_file_header file_hdr; // File header, always present
	struct pe_optional_header optional_hdr; // Optional header, present if
						// file_hdr.optional_header_size
						// is greater than 0
	struct pe_data_directory
		data_directory[PE_DATA_DIR_MAX_ENTRIES]; // The data directory
};
...
enum pe_section_flags {
	...
	PE_SECTION_UNINITIALIZED = 0x80, // The section contains uninitialized
					 // data
	PE_SECTION_MANY_RELOCS = 0x1000000, // The section contains more than
					    // UINT16_MAX relocation entries
...
struct pe_section_header {
	...
	uint32_t line_numbers; // RVA of the line number information of the
			       // section
	uint16_t n_relocs; // Number of relocations in the section
	uint16_t n_line_numbers; // Number of line numbers stored in the
				 // section
...
/// Locates a symbol in a Portable Executable
extern void *purpl_get_pe_symbol(void *base, const char *name);
